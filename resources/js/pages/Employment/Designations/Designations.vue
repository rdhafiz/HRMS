<template>
  <div>
    <div class="flex items-center justify-between mb-4">
      ...
      <router-link v-if="canManage" to="/designations/create" class="px-3 py-2 rounded bg-indigo-600 text-white text-sm">Create</router-link>
    </div>
    <div class="flex items-center gap-2 mb-3">
      <input v-model="q" @input="fetch" placeholder="Search..." class="border rounded px-3 py-2 w-64" />
      <select v-model="departmentId" @change="fetch" class="border rounded px-3 py-2">
    ...
    <div class="bg-white rounded shadow divide-y">
      ...
      <div v-for="d in items" :key="d.id" class="grid grid-cols-3 px-4 py-3 items-center">
        ...
        <div class="text-sm text-gray-600">{{ d.department?.name }}</div>
        <div class="text-right">
          <router-link v-if="canManage" :to="`/designations/${d.id}/edit`" class="text-indigo-600 text-sm mr-2">Edit</router-link>
...
async function fetch(page = 1) {
  const { data } = await axios.get('/employment/designations', { params: { page, q: q.value, department_id: departmentId.value } })
...
async function loadDepartments() {
  const { data } = await axios.get('/employment/departments', { params: { page: 1 } })
